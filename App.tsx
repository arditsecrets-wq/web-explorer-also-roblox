...
const App: React.FC = () => {
  ...
  const PROXY_NODES = [
    { name: 'Node Alpha (Mathematics)', url: 'https://mathematics.life/' },
    { name: 'Node Beta (Interstellar)', url: 'https://interstellar.rip/' },
    { name: 'Node Gamma (Shuttle)', url: 'https://shuttle.rip/' }
  ];
    ...
    <div className={`flex flex-col flex-1 bg-[#020202] text-white ${selectedGame || isMarketplaceOpen ? 'overflow-hidden' : ''}`}>
      <nav className="sticky top-0 z-[60] bg-black/95 border-b border-white/5 px-8 py-5 flex items-center justify-between">
        <div className="flex items-center gap-4 cursor-pointer" onClick={() => setActiveTab('vault')}>
          <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center font-black italic shadow-xl shadow-blue-600/30 text-white text-lg transition-transform hover:scale-110">R</div>
        ...
        <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10">
          ...
          <button onClick={() => setActiveTab('discover')} className={`px-8 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'discover' ? 'bg-white text-black shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}>Games</button>
        </div>

        <div className="hidden lg:flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></div>
          <span className="text-[10px] font-black text-gray-700 uppercase tracking-widest">Gateway Ready</span>
        </div>
      </nav>
          ...
          <div className="max-w-2xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-500">
            <div className="bg-[#080808] border border-white/10 rounded-[2.5rem] p-10 shadow-2xl">
               <div className="text-center mb-10">
                 <h2 className="text-4xl font-black uppercase italic mb-2 tracking-tighter">Secure <span className="text-blue-500">Entry</span></h2>
                 <p className="text-xs text-gray-500 font-bold uppercase tracking-[0.4em]">Integrated Stealth Tunnel</p>
               </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
                  <button onClick={() => unblockLaunch(CLOUD_MOON_URL)} className="group p-8 bg-white/5 border border-white/5 rounded-[2rem] text-left hover:bg-white/10 transition-all">
                     ...
                     <p className="text-xs text-gray-500 font-bold mt-2 uppercase tracking-widest">Direct Virtual VM</p>
                  </button>
                  ...
                  <button onClick={() => unblockLaunch('https://google.com')} className="group p-8 bg-white/5 border border-white/5 rounded-[2rem] text-left hover:bg-white/10 transition-all">
                     ...
                     <p className="text-xs text-gray-500 font-bold mt-2 uppercase tracking-widest">Encrypted Tunnel</p>
                  </button>
               </div>

               <div className="flex flex-col items-center pt-10 border-t border-white/5">
                 <p className="text-xs text-gray-600 font-black uppercase tracking-[0.2em] mb-4">CloudMoon Link (Copy & Paste)</p>
                 ...
                 <p className="mt-8 text-[10px] text-gray-800 font-black uppercase tracking-[0.4em]">Current Relay Node: <span className="text-blue-600">{activeProxy}</span></p>
               </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'studio' && (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-500">
             <div className="bg-[#080808] border border-white/10 rounded-[2.5rem] p-12 shadow-2xl max-w-4xl mx-auto">
                <div className="mb-10">
                  <h3 className="text-3xl font-black uppercase italic mb-3 text-blue-500">Node <span className="text-white">Settings</span></h3>
                  <p className="text-sm text-gray-500 leading-relaxed font-bold uppercase tracking-widest">Switch relay nodes if sites stop loading.</p>
                </div>
        ...
        {activeTab === 'discover' && (
          <div className="space-y-16 animate-in fade-in slide-in-from-bottom-6 duration-500">
            <section className="rounded-[3rem] bg-gradient-to-br from-blue-900/30 to-black border border-white/10 p-20 text-center shadow-2xl">
              <h2 className="text-6xl font-black italic uppercase mb-4 tracking-tighter">Direct <span className="text-blue-500">Access</span></h2>
              <p className="text-lg text-gray-400 font-bold uppercase mb-12 tracking-[0.4em]">Independent Global Gaming Hub</p>
      ...
      <footer className="py-20 text-center border-t border-white/5 opacity-30">
        <p className="text-[10px] font-black text-gray-700 uppercase tracking-[1em]">&copy; 2025 HUB NETWORK V9</p>
      </footer>
